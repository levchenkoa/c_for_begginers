...
int main(int argc, char **argv)
{
    ...
    void parse_agruments(int argc, char **argv, int *numeric, int *reverse);

    int nlines;
    int numeric;
    int reverse;

    parse_agruments(argc, argv, &numeric, &reverse);
    cmpfunc = get_cmpfunc(numeric);

    if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {
        my_qsort((void **) lineptr, 0, nlines-1, cmpfunc, reverse);
